...
namespace s3d
{
	...
	CKeyboard_Linux::CKeyboard_Linux()
	{
	...
	bool CKeyboard_Linux::pressed(const uint32 index) const
	{
		return m_states[index].pressed;
	}
	
	bool CKeyboard_Linux::up(const uint32 index) const
	{
		return m_states[index].up;
	}

	MillisecondsF CKeyboard_Linux::pressedDuration(const uint32 index) const
	{
